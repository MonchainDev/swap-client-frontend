...
import { CurrencyAmount, Price } from '@wicchain/swap-sdk-core'
import { encodeSqrtRatioX96, nearestUsableTick, Pool, Position, priceToClosestTick, TICK_SPACINGS, TickMath, type FeeAmount } from '@wicchain/v3-sdk'
...
export default function useV3DerivedInfo() {
  ...
  //     console.log('🚀 ~ watchEffect ~ currentPrice:', currentPrice)
  //     if (currentPrice) {
  ...
  watch(
    ...
    (fee, oldFee) => {
      ...
      if (fee !== oldFee) {
        const currentPrice = price.value ? parseFloat((invertPrice.value ? price.value.invert() : price.value).toSignificant(8)) : undefined
        if (currentPrice) {
          dispatchRangeTypedValue('BOTH', currentPrice, ZOOM_LEVELS[fee as FeeAmount])
        }
      }
    }
  )
