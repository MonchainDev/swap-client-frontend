...
export class Trade<TInput extends TokenCurrency, TOutput extends TokenCurrency, TTradeType extends TradeType> {
  ...
  public static async bestTradeExactIn<TInput extends TokenCurrency, TOutput extends TokenCurrency>(
    ...
    for (let i = 0; i < pools.length; i++) {
      ...
      if (amountOut.currency.isToken && amountOut.currency.equals(tokenOut)) {
        sortedInsert(
          bestTrades,
          await Trade.fromRoute(new RouteV3([...currentPools, pool], currencyAmountIn.currency, currencyOut), currencyAmountIn, TradeType.EXACT_INPUT),
          maxNumResults,
          tradeComparator
        )
  ...
  public static async swap(
    ...
    while (state.amountSpecifiedRemaining !== ZERO && state.sqrtPriceX96 != sqrtPriceLimitX96) {
      ...
      ;[step.tickNext, step.initialized] = await tickDataProvider.nextInitializedTickWithinOneWord(state.tick, zeroForOne, this.tickSpacing(fee))
