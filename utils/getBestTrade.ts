...
import { FEE_ALLOWANCE } from '~/utils/constant';
import { CurrencyAmount, Token, TradeType, Percent, type Currency } from '@pancakeswap/swap-sdk-core';
import { TICK_SPACINGS, TickMath,
    Pool as V3PoolSDK,
    Route as V3Route,
    Trade as V3Trade,
    type FeeAmount } from '@pancakeswap/v3-sdk';
import invariant from 'tiny-invariant';
import { PoolType, RouteType, type SmartRouterTrade, type V3Pool } from '@pancakeswap/smart-router';
...
export const getBestTrade = async ({token0, token1, inputAmount}: SwapInput) => {
    ...
    for (const fee of FEE_ALLOWANCE) {
     ...
     const trade = await makeV3Trade(listPool, token0Currency, token1Currency, CurrencyAmount.fromRawAmount(token0Currency, BigInt(inputAmount)), TradeType.EXACT_INPUT)
...
// v3
const makeV3Pool = (token0: Token, token1: Token, liquidityNum: bigint, feeAmount: FeeAmount, sqrtRatioX96: bigint, feeProtocol: number, poolAddress: Address): V3Pool => {
