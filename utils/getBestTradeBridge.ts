...
import { CurrencyAmount, type TradeType, type Currency, type Token } from '@monchain/swap-sdk-core'
...
export const getBestTradeBridge = async ({ token0, token1, inputAmount, type, chainId }: SwapInput): Promise<SwapOutput> => {
  try {
    ...
    const bestTrades = await _getBestTrade(token0Currency, token1Currency, currencyAmount, type, client)
    if (!bestTrades) {
      throw new Error('No trade found')
    }
    ...
    for (let i = 0; i < bestTrades.length; i++) {
      ...
      if (bestTradeInAmount.length === 0) {
        throw new Error('bestTradeInAmount is empty')
      }
