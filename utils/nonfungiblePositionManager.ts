import type { BigintIsh, Currency, NativeCurrency, Percent, Token } from '@pancakeswap/sdk'
...
export abstract class NonfungiblePositionManager {
  ...
  public static encodeRefundMON(): `0x${string}` {
    return encodeFunctionData({ abi: NonfungiblePositionManager.ABI, functionName: 'refundMON' })
  }
  ...
  public static addCallParameters(position: Position, options: AddLiquidityOptions): MethodParameters {
    ...
    if (options.useNative) {
      ...
      if (wrappedValue > ZERO) {
        calldatas.push(this.encodeRefundMON())
      }
  ...
  public static removeCallParameters(position: Position, options: RemoveLiquidityOptions): MethodParameters {
    ...
    if (options.liquidityPercentage.equalTo(ONE)) {
      if (options.burnToken) {
        calldatas.push(encodeFunctionData({ abi: NonfungiblePositionManager.ABI, functionName: 'burn', args: [tokenId] }))
      }
    } else {
