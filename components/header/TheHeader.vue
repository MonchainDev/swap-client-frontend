    ...
    <div class="flex h-full items-center justify-between sm:h-fit sm:pt-3">
      <div class="flex items-center">
        <div class="flex items-center gap-2">
          ...
          <span class="text-[22px] font-semibold leading-7 text-hyperlink sm:text-base sm:text-white">{{ tokenReward }}</span>
        </div>
        <div class="ml-[68px] flex items-center gap-8 sm:hidden">
          ...
          <NuxtLink
            :class="{ 'active-menu': $route.path.includes('/liquidity') }"
            to="/liquidity/pool"
...
<script setup lang="ts">
  ...
  const tokenReward = computed(() => {
    return TOKEN_REWARDS[ChainId.MON_TESTNET]?.symbol ?? 'ORB'
  })
