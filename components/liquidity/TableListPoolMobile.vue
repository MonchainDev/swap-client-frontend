  ...
  <div class="mx-4 mb-[80px]">
    <div v-for="(item, index) in props.data" :key="index" class="mb-2 rounded-lg bg-white px-4 py-2">
      <div class="wrap-table flex items-center justify-between">
        ...
        <BaseIcon name="three-dot" size="24" class="cursor-pointer" @click="handleDrawerItem(item)" />
        <el-drawer v-model="drawer" direction="btt" :show-close="false" size="370" class="drawer-liquid rounded-tl-md rounded-tr-md shadow">
          <template #header>
          ...
          <template #default>
            <div v-if="poolDetail" class="px-4 py-5">
              ...
              <div class="mt-3 flex items-center gap-2" @click="setOpenPopup('popup-connect', true)">
...
<script setup lang="ts">
  ...
  const poolDetail = ref<IPool>()

  const drawer = ref<boolean>(false)
  const handleDrawerItem = (item: IPool) => {
    drawer.value = true
    poolDetail.value = item
  }
