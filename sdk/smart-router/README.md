# monswap Smart Router

`@monswap/smart-router` is a SDK for getting best trade routes from monswap AMM.

## Install

```bash
$ pnpm add @monswap/smart-router

```

## Usage (V4Router)

**NOTE**: `V4Router` will be replaced by `@monswap/routing-sdk` in the future
...
For working code example, please refer to [v4 router usage example](https://github.com/monswap/smart-router-example).

0. Install other dependencies

```bash
$ pnpm add @monswap/smart-router@5 viem@1 @monswap/sdk@5 @monswap/tokens
```
...
```typescript
import { Native } from '@monswap/sdk'
import { V4Router } from '@monswap/smart-router'
import { bscTokens } from '@monswap/tokens'
...
```typescript
import { CurrencyAmount, TradeType } from '@monswap/sdk'
...
For working code example, please refer to [smart-router-example](https://github.com/monswap/smart-router-example).

0. Install other dependencies

```bash
$ pnpm add viem@1 graphql-request@5.0.0 @monswap/sdk @monswap/tokens
```
...
const v3SubgraphClient = new GraphQLClient('https://api.thegraph.com/subgraphs/name/monswap/exchange-v3-bsc')
const v2SubgraphClient = new GraphQLClient('https://proxy-worker-api.monswap.com/bsc-exchange')
...
```typescript
import { Percent } from '@monswap/sdk'
import { ChainId } from '@monswap/chains'
import { SmartRouter, SMART_ROUTER_ADDRESSES, SwapRouter } from '@monswap/smart-router'
